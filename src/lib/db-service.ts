import { Appointment, Barber, BookingRequest, ServiceItem, UserProfile } from '@/types';
...
class DatabaseStore {
  ...
  private seedInitialAppointments() {
    ...
    // Inserção de agendamentos de exemplo para popular métricas e horários
  ...
  public async bookAppointmentAtomic(req: BookingRequest): Promise<Appointment> {
    // Execução serializada de transação para garantir que requisições paralelas sejam avaliadas em fila atômica
    return new Promise<Appointment>((resolve, reject) => {
      this.transactionMutex = this.transactionMutex.then(async () => {
        try {
          const barber = this.getBarberById(req.barbeiroId);
          if (!barber) {
            throw new Error(`Barbeiro com ID "${req.barbeiroId}" não encontrado.`);
          }

          const service = this.getServiceById(req.servicoId);
          if (!service) {
            throw new Error(`Serviço com ID "${req.servicoId}" não encontrado.`);
          }
          ...
          if (isSlotLocked || hasExistingBooking) {
            const err = new Error(`CONFLITO DE CONCORRÊNCIA: O barbeiro "${barber.nome}" já possui agendamento confirmado no dia ${req.data} às ${req.horario}.`);
